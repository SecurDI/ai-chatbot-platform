...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { expectedState, expectedNonce, code_verifier } = oidcState;

    const { idToken, accessToken, retrievedState, retrievedNonce } = await exchangeCodeForTokens(code as string, code_verifier as string, redirectUri, expectedNonce as string);
    logger.info("Tokens exchanged successfully");

    if (retrievedState !== expectedState) {
      logger.error(`State mismatch: Expected ${expectedState}, Received ${retrievedState}`);
      return NextResponse.redirect(`${url.origin}/login?error=invalid_state`);
    }
    ...
    await createSession(response, user.id, user.role, accessToken);
