...
export async function createSession(
  ...
  try {
    logger.info("🔐 Starting session creation", { userId, entraId, email, role });
    const now = Date.now();
    ...
    logger.info("💾 Storing session in KV", { sessionId, timeout: AUTH_CONFIG.SESSION_TIMEOUT });
    await kv.set(
      `session:${sessionId}`,
      session,
      { ex: AUTH_CONFIG.SESSION_TIMEOUT }
    );
    logger.info("✅ Session stored in KV");

    // Create JWT token
    logger.info("🎫 Creating JWT token", { expiresAt });
    ...
    logger.info("✅ JWT token created", { tokenLength: token.length });
    logger.info("🎉 Session created successfully", { sessionId, userId, email });
